...
Since most phenology forecasts are based on accumulated growing degree days, making forecasts requires information on both observed temperatures (up to date the forecast is made) and forecast temperatures. For observed data we used 4km daily mean temperature from the PRISM dataset, which is updated daily [@prismdata]. These observed data are saved in a netCDF file, which is appended with the most recent data every time the automated forecast is run. For climate forecasts we used the Climate Forecast System Version 2 (CFSv2;  a coupled atmosphere-ocean-land global circulation model) 2-m temperature data, which has a 6-hour timestep and a spatial resolution of 0.25 degrees latitude/longitude [@saha2014]. Forecasts are projected out 9 months from the issue date and are updated every 6 hours. The five most recent climate forecasts are downloaded for each updated phenology forecast to accommodate uncertainty (see Prediction).
...
Figure 2: Screenshot of the forecast presentation website (http://phenology.naturecast.org) showing the February 21, 2019 forecast for the leaf out of *Acer negundo*. In the upper right is the interface for selecting different species, phenophases, or forecast issue dates via drop down menus.

Figure 3: Locations of phenological events which have occured between Jan. 1, 2019 and April XX, 2019 obtained from the USA National Phenology Network (blue circles), and all sampling locations in the same dataset (red points). Six individual plants are highlighted, where the solid line indicates the predicted event date as well as 95% uncertainty for a specified forecast issue date, and the dotted line indicates the observed event date. 

Figure 4: The root mean square error and the average uncertainty of forecasts issued between Dec. 2, 2018 and April XX, 2019 for XX phenological events. 

Figure 5: Distribution of absolute errors (prediction - observed) for XX phenological events for 13 selected issue dates. Labels indicate the mean absolute error (MAE).
