...
####################
# Climate files
current_season_temperature = xr.open_dataset(config['data_folder'] + 'current_season_observations_2018.nc')
...
for date_i, hindcast_issue_date in enumerate(date_range):
    ...
    # Use observed, current season temperature for 2018 up until the day
    # before the issue date
    current_season_start_date = '2017-11-01'
    current_season_dates_to_extract = pd.date_range(current_season_start_date,
                                                    hindcast_issue_date - pd.DateOffset(1))
    current_season_temperature_subset = current_season_temperature.sel(time = current_season_dates_to_extract)
    
    for historic_year_i, historic_year in enumerate(historic_years):
        historic_year_start_date = '{y}-{m}-{d}'.format(y = historic_year if hindcast_issue_date.month < 10 else historic_year-1,
                                                        m = hindcast_issue_date.month,
                                                        d = hindcast_issue_date.day)
        ...
        site_temp = Parallel(n_jobs=8)(delayed(extract_single_site_temp)(clim=historic_climate_member, site=s.copy()) for s in dataframe_chunker(site_info_for_prediction,1))
...
all_hindcast_predictions.to_csv(config['data_folder']+ 'evaluation/hindcast_lta_method_data_2018.csv', index=False)
