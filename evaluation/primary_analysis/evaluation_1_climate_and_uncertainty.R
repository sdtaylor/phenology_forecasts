...
hindcasts = hindcasts %>%
  ...
  rename(primary_model = doy_prediction,
         ...
         ltr_model = doy_prediction_lta) %>%
  gather(model_type, doy_prediction, primary_model, naive_model, ltr_model)
...
yearly_lead_time_errors$model_type = factor(yearly_lead_time_errors$model_type, 
                                            levels = c('naive_model','ltr_model','primary_model'), 
                                            labels = c('Naive','Long-Term Avg','Primary'))
...
ggplot(yearly_lead_time_errors, aes(x=lead_time, y=mae, color=model_type)) + 
  geom_line(size=3) +
  scale_color_manual(values=c("black", "grey60",'#E69F00')) + 
